...
class Auth_model extends CI_Model {
    ...
    var $client_id = "35EE96982D183E1D";
    var $client_secret  = "1EF6DEA96659C264D711A4BC7C622";

    public function check_auth_client(){
        $client_id = $this->input->get_request_header('Client-Service', TRUE);
        $client_secret  = $this->input->get_request_header('Auth-Key', TRUE);
    ...
    {
        $q  = $this->db->select('password,id,email')->from('users')->where('email',$email)->get()->row();
        if($q == ""){
            ...
            if (hash_equals($hashed_password, crypt($password, $hashed_password))) {
               ...
               $this->db->insert('users_authentication',array('user_id' => $id,'token' => $token,'expired_at' => $expired_at));
